...
const SearchBar = ({
  ...
  <Row as="section" className={className}>
    <Column span={3}><QueryInput onBlur={e => setQuery(e.target.value)} defaultValue={query} /></Column>
    <Column span={3}><DatePicker date={occurredBefore} title="From" onChange={setOccurredBefore} /></Column>
    <Column span={3}><DatePicker date={occurredAfter} title="To" onChange={setOccurredAfter} /></Column>
    <Column span={3}><Button danger>Clear</Button><Button primary onClick={fetchingAction}>Apply</Button></Column>
  </Row>
);
